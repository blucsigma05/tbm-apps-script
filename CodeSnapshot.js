...
// ════════════════════════════════════════════════════════════════════
// CodeSnapshot.gs v2 — Thin wrappers calling ThompsonLib
// ════════════════════════════════════════════════════════════════════

function getCodeSnapshotVersion() { return 2; }
...
// Full snapshot with text files + optional Google Docs
...
// Fast snapshot — text files only
function runSnapshotFast() {
  var cfg = _snapshotConfig_();
  if (!cfg.folderId) throw new Error('Set SNAPSHOT_FOLDER_ID in Script Properties.');
  cfg.createDocs = false;
  return ThompsonLib.snapshotCodeToGDrive(cfg);
}
