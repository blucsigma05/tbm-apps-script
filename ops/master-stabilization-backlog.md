...
| 101 | P0 (before #14) | Define minimum viable standard for JJ surfaces — what does "working" look like for SparkleLearning, daily-adventures, sparkle-kingdom? Written as Issue, not chat | Issue created with explicit pass conditions (renders without error, question displays, audio plays, answer recorded) | Issue merged with explicit quality bar | Open |
| 102 | P0 (before #15) | Define minimum viable standard for Buggsy surfaces — HomeworkModule, reading, writing, wolfkid, comic-studio, wolfdome | Issue created with explicit pass conditions | Issue merged with explicit quality bar | Open |
...
| 103 | P0 (before #21) | Inventory every path that can modify production state: scheduled triggers, manual editor runs, clasp push, webhook, direct sheet edits | Grep GASHardening, AuditTrigger, Code.js, triggers list; produce explicit list of mutation paths | Issue closed with complete mutation path inventory | Open |
