...
17. Every new recurring GAS function adds a row to `OPS_TRIGGER_SPEC` in `OpsTriggers.js` in the same PR as the handler. No trigger spec entry = no trigger installed = silent scheduling failure. Carry `rubric-n/a` label only if the function is explicitly not recurring.
