...
## What this is
Google Apps Script + HtmlService system for household finance tracking and family dashboards. 25+ .gs/.html files. Google Sheets is the data layer, Tiller Money syncs bank data, 5 HTML dashboards served via GAS web app.
...
### TAB_MAP
DataEngine.gs owns TAB_MAP. All sheet references go through it. Never hardcode sheet names with emoji prefixes — only in TAB_MAP definitions.
...
## TBM-specific mistakes to avoid
1. Hardcoding sheet names instead of using TAB_MAP
2. Using `getActiveSpreadsheet()` instead of `openById(SSID)`
3. Duplicating constants across files (shared global scope)
4. Writing to a sheet tab owned by another module
5. Creating a new GAS deployment instead of updating the existing one
